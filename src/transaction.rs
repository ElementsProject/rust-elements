...
use secp256k1_zkp::{RangeProof, SurjectionProof, Tweak, ZERO_TWEAK};
...
impl<'tx> PeginData<'tx> {
    ...
    pub fn from_pegin_witness(
        ...
        Ok(PeginData {
            outpoint: prevout,
            value: bitcoin::consensus::deserialize(&pegin_witness[0])
...
impl TxIn {
    ...
    pub fn pegin_prevout(&self) -> Option<bitcoin::OutPoint> {
        if self.is_pegin {
            ...
            Some(bitcoin::OutPoint {
                txid: bitcoin::Txid::from_byte_array(self.previous_output.txid.to_byte_array()),
    ...
    pub fn pegin_data(&self) -> Option<PeginData> {
        self.pegin_prevout()
            .and_then(|p| PeginData::from_pegin_witness(&self.witness.pegin_witness, p).ok())
    }
    ...
    pub fn issuance_ids(&self) -> (AssetId, AssetId) {
        let entropy = if self.asset_issuance.asset_blinding_nonce == ZERO_TWEAK {
            let contract_hash = ContractHash::from_byte_array(self.asset_issuance.asset_entropy);
        ...
        let token_id = AssetId::reissuance_token_from_entropy(
            entropy,
            self.asset_issuance.amount.is_confidential(),
        );
...
impl Transaction {
    ...
    pub fn discount_weight(&self) -> usize {
        ...
        for out in self.output.iter() {
            ...
            let witness_weight =
                VarInt(sp_len as u64).size() + sp_len + VarInt(rp_len as u64).size() + rp_len;
        ...
        let input_weight = self
            ...
            .map(|input| {
                ...
                input.script_sig.len() + if input.has_issuance() {
                    ...
                    let amt_prf_len = input
                        .witness
                        .amount_rangeproof
                        .as_ref()
                        .map(|x| x.len())
                        .unwrap_or(0);
                    let keys_prf_len = input
                        .witness
                        .inflation_keys_rangeproof
                        .as_ref()
                        .map(|x| x.len())
...
mod tests {
    ...
    fn verify_ct() {
        ...
        let btc_asset: confidential::Asset =
            hex_deserialize!("0b37d4818b8ce1df5d3d0b88d140c6848029d6d85fb0f6ee270865caf53d0b82d4");
        let btc_value: confidential::Value =
            hex_deserialize!("094e2cceeb8005ac14b611821c37fca757b47426afb0bb4eabe41c275d3997c046");
        let spk: script::Script =
            hex_deserialize!("16001475f578ed4f7a0103182a6e92942c66350dd949dc");
        ...
        tx.verify_tx_amt_proofs(&secp, &[txout])
            .expect("Verification");
    }
