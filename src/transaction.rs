...
mod tests {
    ...
    fn rangeproof_roundtrip() {
        ...
        // ciborium does not
        let mut bytes_cib = vec![];
        ciborium::ser::into_writer(&rangeproof, &mut bytes_cib).unwrap();
        assert_eq!(bytes_cib, bytes, "ciborium/serde_cbor don't serialize the same");
        let _: RangeProof = ciborium::from_reader(&bytes_cib[..]).unwrap(); // Semantic(None, "invalid type: bytes, expected bytes")
    }
